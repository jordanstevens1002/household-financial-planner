...
class AppsmithExportTests(unittest.TestCase):
    ...
    def test_release_facing_files_are_complete_and_free_of_delivery_numbering(
        self,
    ) -> None:
        ...
        disclaimer_position = readme.index("Nothing generated by this software")
        features_position = readme.index("## Features")
        self.assertLess(disclaimer_position, features_position)
        self.assertIn("not a financial professional", readme)
        self.assertIn(
        ...
        primary_files = (
            ...
            repository / "appsmith/README.md",
            repository / ".github/pull_request_template.md",
            repository / "appsmith/generate_app.py",
            repository / "appsmith/tests/test_export.py",
        )
        numbered_delivery = re.compile(r"\bphase[\s_-]*\d", re.IGNORECASE)
        for path in primary_files:
            self.assertIsNone(numbered_delivery.search(path.read_text(encoding="utf-8")), path)

        self.assertFalse((repository / "PROJECT_PLAN.md").exists())
        self.assertTrue((repository / "docs/archive/project-plan.md").exists())
    ...
    def test_export_does_not_include_unreleased_demo_mode(self) -> None:
        source = EXPORT.read_text(encoding="utf-8")
    ...
    def test_export_uses_maintained_country_and_currency_selectors(self) -> None:
        actions = {
            item["unpublishedAction"]["name"]: item["unpublishedAction"]
            for item in self.application["actionList"]
        }
